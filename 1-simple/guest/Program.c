...
static void fill_input(unsigned char *data, size_t len) {
    static const unsigned char pattern[] = "QEMU syscall filter zlib compression demo payload\n";
    size_t i;
...
int main(void) {
    ...
    if (zcompress_compress(input, INPUT_SIZE, compressed, &compressed_len, Z_BEST_COMPRESSION) !=
        0) {
    ...
    if (zcompress_uncompress(compressed, compressed_len, output, &output_len) != 0) {
